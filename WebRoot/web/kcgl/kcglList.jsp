	...
	<div id="divTable" align="center" style="height:95%">
		<div class="tool">
			<div id="searchDiv" style=" width: 100%; height: 32px;">
				<a onclick="excute('kcglEdit.action')" class="buttonNew btn-red-1" style="width: 46px;"><i class="buttonAdd-icon icon-add"></i>添加</a>
				<div style="float: right;height: 100%;">
					<form id="paginationAndSearchForm" action="kcglListBycondition.action" method="post" class="" >
						<!-- 分页隐藏域 -->
		...
		<table class="table tableNew" cellspacing="0" >
			...
			<tbody>
				...
				<c:forEach var="item" items="${kcglList}" >
				<tr>
				    ...
				    <td>${item.price1}</td>
				   	...
				   	<td>${item.descs}</td>
					<td>
						<button onclick="excute('kcglEdit.action?kcgl.id=${item.id}')" style="width: 40px" class="button button--tamaya button--round-s button--text-thick button--border-thin button--text-upper button--size-s1 button--inverted" data-text="编辑"><span>编辑</span></button>
						<button onclick="excute('kcglDel.action?kcgl.id=${item.id}')" style="width: 40px" class="button button--tamaya button--round-s button--text-thick button--border-thin button--text-upper button--size-s1 button--inverted" data-text="删除"><span>删除</span></button>
					</td>
				</tr>
				</c:forEach>
			</tbody><!-- 表体结束 -->
