...
<!-- 字段验证 -->
<%-- <script type="text/javascript" src="${basePath }/web/kcgl/js/cheakReg.js" charset="UTF-8"></script> --%>
...
function savecheck(){
	...
	var name = $("input[name='kcgl.name']").val();
	if(name==""){
		$("input[name='kcgl.name']").next().html("此项不能为空！");
		flag = false;
	}
	var id = $("input[name='kcgl.id']").val();
	
	$.post("kcglByRepetition.action?date"+Math.random(),{'kcgl.name':name, 'kcgl.id':id},function(data){
		var obj = eval("("+data+")");
		if(obj.flag==false){
			$("input[name='kcgl.name']").next().html("重复！");
			return;
		}else{
			$("#submitForm").submit();
		}
...
<form id="submitForm" class="form-horizontal" action="kcglSave.action" method="post" target="kcglBox">
	<input type="hidden" name="kcgl.id" value="${kcgl.id }">
			...
			<div id="collapseOne" class="panel-collapse collapse in">
				<div class="panel-body" style="padding: 15px 0px 0px 15px;margin-bottom:0px;">
					...
					<div class="form-group">
						<label class="col-sm-2 control-label"><span style="color: #ff0000;">* </span>课程名称</label>
						<div class="col-sm-10">
							<input check="required,notR" style="width:80%" maxlength="32" type="text" name="kcgl.name" class="textInput frequired" value="${kcgl.name}"/>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<div class="item">
								<img class="addImg" onclick="clickImg(this);" src="${basePath }/web/kcgl/images/addImg.png" />
								<input type="file" class="upload_input" onchange="change(this)"  />
								<input id="imgData" name="kcgl.img" type="hidden"  value="${kcgl.img }" />
								<div class="preBlock">
									<img class="preview" id="preview" alt="" name="pic" width="200" height="200" src="${kcgl.img }" />
								</div>
								<img class="delete" onclick="deleteImg(this)" src="${basePath }/web/kcgl/images/delete.png"/>
							</div>
							<span class="info"></span>
						</div>
				    </div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input check="required,notR" style="width:80%" maxlength="32" type="text" name="kcgl.price1" class="textInput frequired" value="${kcgl.price1}"/>
							<span class="info"></span>
						</div>
				    </div> 
				    
					<div class="form-group">
						<label class="col-sm-2 control-label"><span style="color: #ff0000;">* </span>实际价格</label>
						<div class="col-sm-10">
							<input check="required,notR" style="width:80%" maxlength="32" type="text" name="kcgl.price" class="textInput frequired" value="${kcgl.price}"/>
							<span class="info"></span>
						</div>
				    </div>
				    ...
				    <div class="form-group">
							...
							<div class="col-sm-10">
								<textarea maxlength="256" style="width:80%; height: 90px; line-height: 20px; font-size: 14px; padding-top: 3px;" name="kcgl.descs">${kcgl.descs }</textarea>
								<span class="info"></span>
							</div>
					</div>					
		      </div>
