...
<link rel="stylesheet" href="${basePath }/web/double-date/double-date.css"/>
<!-- <script src="http://www.jq22.com/jquery/jquery-1.10.2.js"></script> -->
<script src="${basePath }/web/double-date/double-date.js"></script>
...
function savecheck(){
	...
	var shopname = $("input[name='zymd.shopname']").val();
	if($.trim(shopname)==""){
		$("input[name='zymd.shopname']").next().html("此项不能为空！");
		flag = false;
	}
	var id = $("input[name='zymd.id']").val();
	//alert(shopname);
	$.post("zymdByRepetition.action?date"+Math.random(),{'zymd.shopname':shopname, 'zymd.id':id},function(data){
		var obj = eval("("+data+")");
		if(obj.flag==false){
			$("input[name='zymd.shopname']").next().html("重复！");
			return;
		}else{
			$("#submitForm").submit();
		}
			...
			<div id="collapseOne" class="panel-collapse collapse in">
				<div class="panel-body" style="padding: 15px 0px 0px 15px;margin-bottom:0px;">
					...
					<div class="form-group">
						<label class="col-sm-2 control-label"><span style="color: #ff0000;">* </span>门店名称</label>
						<div class="col-sm-10">
							<input check="required,notR" onblur="checkReg(this);" style="width:80%" maxlength="32" type="text" name="zymd.shopname" class="textInput frequired" value="${zymd.shopname}"/>
				    ...
				    <div class="form-group">
						<label class="col-sm-2 control-label"><span style="color: #ff0000;">* </span>时间</label>
						<div class="col-sm-10">
							<div class="outer clearfix">
							    <div class="date date1 fl" id="from">
							        <input name="zymd.time" type="text" class="date-check" value=""/>
							    </div>
							    <div style="display: none" class="date fr" id="to">
							        <input type="text" class="date-check" value="9999-10-11"/>
							    </div>
							</div>
							<span class="info"></span>
						</div>
