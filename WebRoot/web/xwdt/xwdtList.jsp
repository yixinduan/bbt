	...
	<div id="divTable" align="center" style="height:95%">
		<div class="tool">
			<div id="searchDiv" style=" width: 100%; height: 32px;">
				<a onclick="excute('xwdtEdit.action')" class="buttonNew btn-red-1" style="width: 46px;"><i class="buttonAdd-icon icon-add"></i>添加</a>
				<div style="float: right;height: 100%;">
					<form id="paginationAndSearchForm" action="xwdtListBycondition.action" method="post" class="" >
						<!-- 分页隐藏域 -->
						...
						<ul class="ml-navigation-bar-5" style="margin-top: 0px;margin-bottom: 0px; ">
							<div class="search-field" >
								<input type="text" onblur="this.placeholder = '请输入...';" onfocus="this.placeholder = '';" placeholder="请输入标题或者内容..." name="condition" value="${condition }"/>
		...
		<table class="table tableNew" cellspacing="0" >
			...
			<tbody>
				...
				<c:forEach var="item" items="${xwdtList}" >
				<tr>
				   	<td>${item.title}</td>
				   	<td>${item.content}</td>
				   	...
				   	<td>${item.time}</td>
					<td>
						<button onclick="excute('xwdtEdit.action?xwdt.id=${item.id}')" style="width: 40px" class="button button--tamaya button--round-s button--text-thick button--border-thin button--text-upper button--size-s1 button--inverted" data-text="编辑"><span>编辑</span></button>
						<button onclick="excute('xwdtDel.action?xwdt.id=${item.id}')" style="width: 40px" class="button button--tamaya button--round-s button--text-thick button--border-thin button--text-upper button--size-s1 button--inverted" data-text="删除"><span>删除</span></button>
					</td>
				</tr>
				</c:forEach>
			</tbody><!-- 表体结束 -->
