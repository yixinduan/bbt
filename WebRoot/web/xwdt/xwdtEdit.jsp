...
<!-- 字段验证 -->
<%-- <script type="text/javascript" src="${basePath }/web/xwdt/js/cheakReg.js" charset="UTF-8"></script> --%>
...
function savecheck(){
	...
	var title = $("input[name='xwdt.title']").val();
	if(title.trim()==""){
		$("input[name='xwdt.title']").next().html("此项不能为空！");
		flag = false;
	}
	var id = $("input[name='xwdt.id']").val();
	
	$.post("xwdtByRepetition.action?date"+Math.random(),{'xwdt.title':title, 'xwdt.id':id},function(data){
		var obj = eval("("+data+")");
		if(obj.flag==false){
			$("input[name='xwdt.title']").next().html("重复！");
			return;
		}else{
			$("#submitForm").submit();
		}
...
<form id="submitForm" class="form-horizontal" action="xwdtSave.action" method="post" target="xwdtBox">
	<input type="hidden" name="xwdt.id" value="${xwdt.id }">
			...
			<div id="collapseOne" class="panel-collapse collapse in">
				<div class="panel-body" style="padding: 15px 0px 0px 15px;margin-bottom:0px;">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<div class="item">
								<img class="addImg" onclick="clickImg(this);" src="${basePath }/web/xwdt/images/addImg.png" />
								<input type="file" class="upload_input" onchange="change(this)"  />
								<input id="imgData" name="xwdt.img" type="hidden"  value="${xwdt.img }" />
								<div class="preBlock">
									<img class="preview" id="preview" alt="" name="pic" width="200" height="200" src="${xwdt.img }" />
								</div>
								<img class="delete" onclick="deleteImg(this)" src="${basePath }/web/xwdt/images/delete.png"/>
							</div>
					...
					<div class="form-group">
						<label class="col-sm-2 control-label"><span style="color: #ff0000;">* </span>标题</label>
						<div class="col-sm-10">
							<input check="required,notR" onblur="checkReg(this);" style="width:80%" maxlength="32" type="text" name="xwdt.title" class="textInput frequired" value="${xwdt.title}"/>
							<span class="info"></span>
						</div>
				    </div>
				    
				    <div class="form-group">
							<label class="col-sm-2 control-label"><span style="color: #ff0000;"></span>内容</label>
							<div class="col-sm-10">
								<textarea onblur="checkReg(this);" maxlength="256" style="width:80%; height: 90px; line-height: 20px; font-size: 14px; padding-top: 3px;" name="xwdt.content">${xwdt.content }</textarea>
								<span class="info"></span>
							</div>
					</div>					
		      </div>
