	...
	<div id="divTable" align="center" style="height:95%">
		...
		<div class="tool">
			<div id="searchDiv" style=" width: 100%; height: 32px;">
				...
				<div style="float: right;height: 100%;">
					<form id="paginationAndSearchForm" action="kclyListBycondition.action" method="post" class="" >
						<!-- 分页隐藏域 -->
		...
		<table class="table tableNew" cellspacing="0" >
			...
			<tbody>
				...
				<c:forEach var="item" items="${kclyList}" >
				<tr>
				   	<td>${item.kcgl.name}</td>
				   	<td>${item.user.name}</td>
				   	<td>${item.content}</td>
					<td>
						<button onclick="excute('kclyEdit.action?kcly.id=${item.id}')" style="width: 40px" class="button button--tamaya button--round-s button--text-thick button--border-thin button--text-upper button--size-s1 button--inverted" data-text="编辑"><span>回复</span></button>
						<button onclick="excute('kclyDel.action?kcly.id=${item.id}')" style="width: 40px" class="button button--tamaya button--round-s button--text-thick button--border-thin button--text-upper button--size-s1 button--inverted" data-text="删除"><span>删除</span></button>
					</td>
				</tr>
				</c:forEach>
			</tbody><!-- 表体结束 -->
