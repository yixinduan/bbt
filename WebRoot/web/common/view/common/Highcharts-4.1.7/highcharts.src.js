...
Pointer.prototype = {
	...
	onContainerMouseMove: function (e) {
		...
		if ((this.inClass(e.target, 'highcharts-tracker') || 
				chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop)) && !chart.openZymd) {
...
extend(Highcharts.Pointer.prototype, {
	...
	touch: function (e, start) {
		...
		if (e.touches.length === 1) {
			...
			if (chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop) && !chart.openZymd) {
...
Chart.prototype = {
	...
	getContainer: function () {
		...
		chart.container = container = createElement(DIV, {
				...
				position: RELATIVE,
				overflow: HIDDEN, // needed for context zymd (avoid scrollbars) and
	...
	translate: function () {
		...
		each(series.points, function (point) {
			...
			if (mathAbs(barH) < minPointLength) {
				if (minPointLength) {
					if ( barY != yBottom ) { // 数据太小显示不出，但为0的不显示。up by zxf 2015.12.31
						barH = minPointLength;
